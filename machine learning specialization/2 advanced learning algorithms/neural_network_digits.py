...
class Layer():
    ...
    def init_W_B(self, num_inputs: int):
        ...
        # self.W = np.zeros((num_inputs, self.units))
...
X = np.load("./X_0_1_only_shuffled.npy")
Y = np.load("./Y_0_1_only_shuffled.npy")
...
model = Sequential(
    [
        ...
        Layer(units=2, activation="linear")
    ],
...
print(f"=========================================================")
start = datetime.now()
model.train(Xn, Y, alpha, iterations=epochs, epsilon=epsilon, adam=adam, lambda_=lambda_, verbose=verbose)
end = datetime.now()
print(f"=========================================================")
output = np.argmax((model.propagate(X_tstn) >= 0.5), axis=1)
